...
type PlanSummary = (typeof planPresets)[number] & {
  total: number
  baseTotal: number
...
export function DashboardShell({
  ...
  const bundleDiscountAmount = useMemo(() => {
    if (!hasBundleDiscount || baseMonthlyTotal <= 0) {
      return 0
    }

    const discount = baseMonthlyTotal * BUNDLE_DISCOUNT_RATE
    return Math.round(discount * 100) / 100
  }, [baseMonthlyTotal, hasBundleDiscount])

  const monthlyTotal = useMemo(() => {
    if (!hasBundleDiscount) {
      return baseMonthlyTotal
    }

    const discountedTotal = baseMonthlyTotal - bundleDiscountAmount
    return Math.round(Math.max(discountedTotal, 0) * 100) / 100
  }, [baseMonthlyTotal, bundleDiscountAmount, hasBundleDiscount])
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-3">
          ...
          <aside className="space-y-6">
            <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-slate-900">
              ...
              {bundleDiscountAmount > 0 ? (
                <p className="mt-2 text-xs font-medium text-green-600 dark:text-green-300">
                  10% bundle savings applied automatically.
                </p>
              ) : null}
              {planName && !activePlan ? (
                <p className="mt-1 text-xs tracking-wide text-indigo-600 uppercase dark:text-indigo-300">
              ...
              <div className="mt-6 space-y-2 text-sm">
                <div className="flex justify-between text-gray-600 dark:text-gray-400">
                  <span>Subtotal</span>
                  <span>${baseMonthlyTotal.toFixed(2)}</span>
                </div>
                {bundleDiscountAmount > 0 ? (
